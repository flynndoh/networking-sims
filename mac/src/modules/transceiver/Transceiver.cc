...
void Transceiver::initialize() {
    ...
    csTimeMessage = new cMessage("csTimeMessage");
    turnAroundToTransmitCompleted = new cMessage("turnAroundToTransmitCompleted");
    turnAroundToReceiveCompleted = new cMessage("turnAroundToReceiveCompleted");
    endOfTransmissionMessage = new cMessage("endOfTransmissionMessage");
}
...
void Transceiver::finish() {
    ...
    EV << HERE << "currentTransmissions.size() = " << currentTransmissions.size() << endl;
...
void Transceiver::handleSelfMessage(cMessage *msg) {
    if (msg == csTimeMessage) {
        EV << HERE << "INFO: received self-message csTimeMessage." << endl;
        processCarrierSense();
        return;
    }

    if (msg == turnAroundToTransmitCompleted) {
        EV << HERE << "INFO: received self-message turnAroundToTransmitCompleted." << endl;
        processTurnAroundToTransmitCompleted();
        return;
    }

    if (msg == turnAroundToReceiveCompleted) {
        EV << HERE << "INFO: received self-message turnAroundToReceiveCompleted." << endl;
        processTurnAroundToReceiveCompleted();
        return;
    }

    if (msg == endOfTransmissionMessage) {
        EV << HERE << "INFO: received self-message endOfTransmissionMessage." << endl;
...
void Transceiver::handleSignalStart(SignalStart *msg) {
    if (transceiverState != TXRX_STATE_RECEIVE) {
        EV << HERE << "WARNING: Received SignalStart when transceiver isn't in receive state. Moving on." << endl;
        delete msg; // prevent memory leaks
        return;
    }

    EV << HERE << "INFO: handling an incoming SignalStart packet" << endl;
    ...
    if (currentTransmissions.find(msg->getIdentifier()) != currentTransmissions.end()) {
        EV << HERE << "ERROR: Detected new signal transmission for an already transmitting Node." << endl;
        error("Aborted, check logs.");
    }

    // Check for collisions on the channel.
    if (currentTransmissions.size() > 0) {
        EV << HERE << "Marked SignalStart message with id " << msg->getIdentifier() << " as collided" << endl;
        msg->setCollidedFlag(true);
        markAllMessagesAsCollided();
    }

    // Add signal to the transmissions map.
    EV << HERE << "Mapping " << msg->getIdentifier() << " to " << msg << endl;
...
SignalStart *Transceiver::getTransmittingSignalWithId(int signalId, bool andRemove) {
    ...
    if (andRemove) {
        EV << HERE << "Removing SignalStart message with id " << signalId << endl;
...
void Transceiver::handleSignalStop(SignalStop *msg) {
    if (transceiverState != TXRX_STATE_RECEIVE) {
        EV << HERE << "WARNING: Received SignalStop when transceiver isn't in receive state. Moving on..." << endl;
        delete msg; // prevent memory leaks
        return;
    }

    EV << HERE << "INFO: handling an incoming SignalStop packet with id " << msg->getIdentifier() << endl;
    SignalStart *storedSignalStart = getTransmittingSignalWithId(msg->getIdentifier(), true);

    if (storedSignalStart == nullptr) {
        EV << HERE << "ERROR: Attempted to stop a signal that wasn't being transmitted from any Node." << endl;
        error("Aborted, check logs.");
    }
    ...
    if (storedSignalStart->getCollidedFlag()) {
        EV << HERE << "INFO: Detected a collision, moving on." << endl;
    } else {
        EV << HERE << "INFO: No collisions present, processing..." << endl;
        ...
        if ((1 - packetErrorProbability) >= uniformDistribution01) {
            EV << HERE << "INFO: Packet is NOT erroneous (passed the random check), processing..." << endl;
            ...
            if (storedMacMessage->getReceiverNodeId() == parentNodeId) {
                EV << HERE << "INFO: MacMessage's destination is this node, processing..." << endl;

                // Encapsulate into a message of type TransmissionIndication and send to local MAC.
                cPacket *transmissionIndication = new TransmissionIndication();
                ...
                EV << HERE << "INFO: MacMessage's destination is NOT this node, moving on." << endl;
                delete storedMacMessage;
            }

        } else {
            EV << HERE << "INFO: Packet is erroneous (failed the random check), moving on." << endl;
            delete storedMacMessage;
        }
    }
...
void Transceiver::handleCSRequest(CSRequest *msg) {
    EV << HERE << "INFO: handling an incoming CSRequest packet." << endl;
    ...
    if (transceiverState == TXRX_STATE_RECEIVE) {
        ...
        EV << HERE <<"The time is now " << simTime() << " and csTimeMessage is scheduled for " << csTime << " seconds time." << endl;
        scheduleAt(simTime() + csTime, csTimeMessage);
    } else if (transceiverState == TXRX_STATE_TRANSMIT || transceiverState == TXRX_STATE_TURNING_AROUND) {
        EV << HERE << "WARNING: Received CSRequest when transceiver is in transmit or turning around state!" << endl;
...
void Transceiver::handleTransmissionRequest(TransmissionRequest *msg) {
    if (transceiverState == TXRX_STATE_RECEIVE) {
        ...
        EV << HERE << "INFO: Starting turn around from receive to transmit state..." << endl;
        scheduleAt(simTime() + turnAroundTime, turnAroundToTransmitCompleted);
        ...
        if (transceiverState == TXRX_STATE_TRANSMIT) {
            ...
            EV << HERE << "WARNING: Received TransmissionRequest while transceiver state is TURNING_AROUND!!" << endl;
        }
...
void Transceiver::processStartOfTransmission() {
    ...
    scheduleAt(simTime() + (packetLengthBits / bitRate), endOfTransmissionMessage);
}
...
void Transceiver::processEndOfTransmission() {
    ...
    // Wait for turnAroundTime parameter.
    transceiverState = TXRX_STATE_TURNING_AROUND;
    EV << HERE << "INFO: Starting turn around from transmit to receive state..." << endl;
    scheduleAt(simTime() + turnAroundTime, turnAroundToReceiveCompleted);
}

void Transceiver::processTurnAroundToTransmitCompleted() {
    EV << HERE << "INFO: Successfully entered transmit state." << endl;
    transceiverState = TXRX_STATE_TRANSMIT;
    processStartOfTransmission();
}

void Transceiver::processTurnAroundToReceiveCompleted() {
    EV << HERE << "INFO: Successfully entered receive state." << endl;
    transceiverState = TXRX_STATE_RECEIVE;
    TransmissionConfirm *transmissionConfirm = new TransmissionConfirm();
